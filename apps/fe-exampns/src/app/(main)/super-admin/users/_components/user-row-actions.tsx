...
function StatusActionButton({
  ...
  const [state, formAction, isPending] = useActionState(updatePlatformUserStatusAction, initialAdminActionState);
...
function DeleteUserDialog({ user }: { readonly user: AdminUserItem }) {
  ...
  const [state, formAction, isPending] = useActionState(deletePlatformUserAction, initialAdminActionState);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <p className="text-sm">
            Hapus <strong>{user.fullName}</strong> ({user.email})? Data di-soft-delete dan tidak bisa login lagi.
          </p>
