...
import { Cell, Pie, PieChart, Bar, BarChart, CartesianGrid, Line, LineChart, XAxis, YAxis } from "recharts";
...
export function RevenueLineChart({
  ...
  readonly data: ReadonlyArray<{ label: string; revenue: number }>;
}) {
  return (
    <ChartContainer
      className="h-72 w-full"
  ...
  return (
    ...
    >
      <PieChart>
        ...
        />
        <Pie data={data} dataKey="value" nameKey="name" innerRadius={65} outerRadius={96} paddingAngle={2} strokeWidth={0}>
          {data.map((segment) => (
        ...
        <ChartLegend
          ...
          content={<ChartLegendContent className="flex flex-wrap gap-3 pt-4" />}
        />
      </PieChart>
    </ChartContainer>
  );
...
export function UsersGrowthLineChart({
  data,
}: {
  readonly data: ReadonlyArray<{ label: string; users: number }>;
}) {
  return (
    <ChartContainer
