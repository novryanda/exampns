...
function getSubItemGroup(groupLabel: string | undefined, itemTitle: string) {
  return itemTitle ? (groupLabel ?? "Other") : "Other";
}
...
export function SearchDialog({ userRole }: { readonly userRole?: string }) {
  const [open, setOpen] = React.useState(false);
  const [query, setQuery] = React.useState("");
  const router = useRouter();
  const searchItems = React.useMemo(() => buildSearchItems(userRole), [userRole]);
  const recommendations = React.useMemo(
    () => searchItems.filter((item) => !item.disabled && !item.url.includes("coming-soon")),
    [searchItems],
  );
